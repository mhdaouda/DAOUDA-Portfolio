...
(function () {
    ...
    function esc(s) {
        if (window.PortfolioAPI?.escapeHtml) return PortfolioAPI.escapeHtml(s);
        if (!s) return '';
    ...
    async function loadContacts() {
        const res = await PortfolioAPI.fetchDashboard();
    ...
    function initDemoMail() {
        ...
        window.PortfolioAPI = window.PortfolioAPI || {};
        PortfolioAPI.mailList = async () => ({ ok: true, campaigns });
        PortfolioAPI.mailGet = async (id) => ({
            ok: true,
            campaign: campaigns.find((c) => c.id === id),
            recipients: (window.DEMO_MAIL_RECIPIENTS || {})[id] || []
        });
        PortfolioAPI.mailCreate = async (p) => {
        ...
        PortfolioAPI.mailSend = async (id) => {
        ...
        PortfolioAPI.mailDelete = async (id) => {
            campaigns = campaigns.filter((c) => c.id !== id);
            return { ok: true };
        };
        PortfolioAPI.mailTest = async () => ({ ok: true, test: true });
        PortfolioAPI.fetchDashboard = async () => ({ ok: true, contacts });
